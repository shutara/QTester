...
uses Unit3;
{$R *.dfm}
procedure TForm1.N6Click(Sender: TObject); //Інформація
begin
  Application.MessageBox('Прорама QTester.' + #13#10 +
    'Автор Шут Андрій.' + #13#10 + '1-й Курс К12-11 ЕМП', 'Інформація', MB_OK +
    MB_ICONINFORMATION + MB_TOPMOST);
end;
procedure TForm1.N7Click(Sender: TObject); //Перехід на сайт
begin
  ShellExecute(Handle, 'open', 'http://sav-dev.blogspot.com', nil, nil, SW_SHOWNORMAL);
end;
procedure TForm1.N8Click(Sender: TObject); //Вихід з програми
begin
  remfiles;
  Application.Terminate;
end;
procedure TForm1.menu(var tag:integer); //Кнопка Заліку №1 (Активна)
var
  ini:TIniFile;
  m:TMemoryStream;
  i:Integer;
...
procedure TForm1.FormCreate(Sender: TObject); //Запуск ПО з перевірками
label
  chk;
var
  m,m2: TMemoryStream;
...
begin
  ...
  begin
    block:=true;
    ShowMessage('Доступ до тестування з цієї машини заблоковано, зв"язжіться з адмінітсратором.');
    remfiles;
    Application.Terminate;
    end;
  ...
  begin
    case
      Application.MessageBox('Хочете створити акаунт? Чи працювати в тимчасовому?'
      + #13#10 + 'Так для створення, Ні для тимчасового.',
      'Файл Налаштуваннь не знайдено', MB_YESNO + MB_ICONQUESTION + MB_TOPMOST)
      of
      IDYES:
        begin
          ...
          way := ExtractFilePath(Application.ExeName) + 'settings.ini';
          Ini := tinifile.create(way);
          user := InputBox('Увага!', 'Введіть прізвище опитуваного.', '');
          vuzname := InputBox('Увага', 'Введіь назву вашого ВНЗ, Всі літери з великої.', 'KIBIT');
          kurs := InputBox('Увага', 'Номер вашого курсу', '1');
...
procedure TForm1.var1Click(Sender: TObject); //Кнопка 1-го варіанту
begin
  if pr = False then
  begin
    if v1 = true then
    ...
    lbl2.Caption := 'Бали: ' + IntToStr(p);
  end
  else
  begin
  end;
end;
procedure TForm1.var2Click(Sender: TObject); //Кнопка 2-го варіанту
begin
  if pr2 = False then
  begin
    if v2 = true then
...
procedure TForm1.var3Click(Sender: TObject); //Кнопка 3-го варіанту
begin
  if pr3 = False then
  begin
    if v3 = true then
...
procedure TForm1.var4Click(Sender: TObject); //Кнопка 4-го варіанту
begin
  if pr4 = False then
  begin
    if v4 = true then
...
procedure TForm1.var5Click(Sender: TObject); //Кнопка 5-го варіанту
begin
  if pr5 = False then
  begin
    if v5 = true then
...
begin
  ...
  if s3 = 27 then
  begin
    btn3.Caption := 'Завершити тест';
  end;
end;
procedure TForm1.tmr1Timer(Sender: TObject); //Відлік Часу
var
  m0, s0: Integer;
begin
  s := s + 1;
  ...
  M0 := 19 - M;
  s0 := 60 - s;
  lbl4.Caption := 'Час ' + inttostr(M0) + ':' + inttostr(S0);
  if m = 20 then
  begin
    s3 := 28;
    tmr1.Enabled := False;
...
procedure TForm1.edt1KeyPress(Sender: TObject; var Key: Char); //Консоль
begin
  if (Key = #27) then
  begin
    edt1.Hide;
  ...
  begin
    ...
    begin
      ...
      n2.Enabled := True;
      N3.Enabled := True;
      edt1.Hint := 'Якщо не бажаєте продовжувати, натисніть Escape';
    end;
    if edt1.Text = 'pmax' then
    begin
      p := 52;
      edt1.Hint := 'Получено Максимум Очков!';
      edt1.Text := '';
    end;
    ...
    begin
      ...
      begin
        tmr1.Enabled := False;
        edt1.Hint := 'Время Заморожено!';
        edt1.Text := '';
      end
      else
      begin
        tmr1.Enabled := true;
        edt1.Hint := 'Время запущено!';
        edt1.Text := '';
      end;
    end;
...
procedure TForm1.btn3KeyPress(Sender: TObject; var Key: Char); //Дебаг
begin
  if (Key = 'C') then
  begin
    edt1.Show;
...
function tform1.FTPFileExists(const RemoteFile: string): Boolean; //Функція перевірки наявності файлу на сервері
begin
  IdFTP2.List(nil, '-la ' + RemoteFile);
  Result := IdFTP2.DirectoryListing.Count > 0;
end;
procedure TForm1.tmr3Timer(Sender: TObject); //Виведення оцінки
var
  way: string;
  me: TMemoryStream;
  res: real;
...
begin
  ...
  begin
    ...
    begin
      ...
      form1.Hide;
      tmr3.enabled := false;
      ShowMessage('Тест пройдено ! Кількість балів: ' + floattostr(res) + '. Результати збережено!');
...
procedure TForm1.N9Click(Sender: TObject); //Видалення тесту (Локально)
begin
  dlgOpen1.Execute;
  DeleteFile(dlgOpen1.FileName);
  showmessage('Тест Було Видалено');
end;
procedure TForm1.N2Click(Sender: TObject); //Редагування
begin
  if N3.Enabled = False then
  begin
    case
      Application.MessageBox('Ви хочете почати редагування. Введіть пароль доступу для продовженння та натисніть Enter. Ви хочете продовжити?',
...
procedure TForm1.N4Click(Sender: TObject);   //Створення тесту
begin
  form1.Hide;
  form2.show;
  Form2.Caption := 'Редагування';
  name_t := InputBox('Увага!', 'Введіть назву тесту', '');
  Form2.Caption := 'Редагування ' + name_t;
end;
procedure TForm1.FormClose(Sender: TObject; var Action: TCloseAction); //Закриття Форми
begin
remfiles;
  if idftp2.Connected = True then
  begin
...
procedure TForm1.tmr4Timer(Sender: TObject); //Вивід результатів із надсиланням власних
var
  me: TMemoryStream;
  ini: TIniFile;
...
procedure TForm1.N5Click(Sender: TObject); //Завантаження Нового тесту
begin
  dlgOpen1.Execute;
  ...
  begin
    ...
    try
      ...
      idftp2.disconnect;
      idftp2.Free;
      ShowMessage('Тест завантажено.');
    end;
  end;
end;
procedure TForm1.N14Click(Sender: TObject); //Зміна Данних Аккаунта
var
  ini: TIniFile;
  way: string;
begin
  ...
  vuzname := ini.ReadString('General', 'VUZ', 'Error!');
  kurs := ini.ReadString('General', 'KURS', 'Error!');
  user := InputBox('Увага!', 'Введіть прізвище опитуваного.', user);
  vuzname := InputBox('Увага', 'Введіь назву вашого ВНЗ, Всі літери з великої.', vuzname);
  kurs := InputBox('Увага', 'Номер вашого курсу', kurs);
  ini.WriteString('General', 'User', user);
  ini.WriteString('General', 'VUZ', vuzname);
  ini.WriteString('General', 'KURS', kurs);
  ini.Free;
end;
procedure TForm1.N15Click(Sender: TObject); //Видалення Результатів Тестування
var
  pred:string;
begin
  if idftp2.Connected = False then
  ...
  begin
    ...
    idftp2.ChangeDir(vuzname);
    idftp2.ChangeDir(kurs);
    pred:=inputbox('ID предмету','Наприклад 1,2','');
    ...
    try
      ...
      on e: Exception do
      begin
        ShowMessage('Не існує файлу!' + ' Помилка: ' + e.Message);
        end;
    end;
    idftp2.Disconnect;
    idftp2.Free;
...
procedure tform1.loadmtm;  //Головне меню Дисциплін
var
  M:TMemoryStream;
  ini:TIniFile;
  way:string;
...
procedure TForm1.testing(var id, qid: integer); //Процедура отримання питаннь від енератора і показ їх
var
  way, workdir: string;
  testdata: TIniFile;
  I: Integer;
begin
  ...
  begin
    ...
    if v1 = False then
    begin
      var1.Hint := 'Неправильна відповідь';
    end
    else
    begin
      var1.Hint := 'Правильна відповідь';
    end;
    if v2 = False then
    begin
      var2.Hint := 'Неправильна відповідь';
    end
    else
    begin
      var2.Hint := 'Правильна відповідь';
    end;
    if v3 = False then
    begin
      var3.Hint := 'Неправильна відповідь';
    end
    else
    begin
      var3.Hint := 'Правильна відповідь';
    end;
    if v4 = False then
    begin
      var4.Hint := 'Неправильна відповідь';
    end
    else
    begin
      var4.Hint := 'Правильна відповідь';
    end;
    if v5 = False then
    begin
      var5.Hint := 'Неправильна відповідь';
    end
    else
    begin
      var5.Hint := 'Правильна відповідь';
    end;
  end;
  lbl3.Caption := 'Питання № ' + IntToStr(s3);
end;
procedure TForm1.ex(Sender: TObject; E: Exception);
begin
  count(E);
end;
procedure TForm1.N16Click(Sender: TObject);  //Додавання ВНЗ та курсів до нього
var
  vuzn: string;
  I, mz: integer;
begin
  vuzn := InputBox('Введіть Абривіатуру ВНЗ', 'Наприклад: КНЕУ,КіБіТ,КПІ', '');
  mz := StrToInt(InputBox('Введіть кількість курсів', 'Наприклад: 1,2', '5'));
  idftp2.Host := '176.37.113.152';
  idftp2.Username := 'QTESTER';
  idftp2.Password := '83H3M52G';
  idftp2.Passive := True;
  ...
  begin
    ...
    try
      ...
      on E: Exception do
      begin
        showmessage('Вже є такий вуз! Зупинка створення...');
        idftp2.Disconnect;
        Exit;
      end;
    end;
    ...
    begin
      try
        ...
        on e: Exception do
        begin
          showmessage('Вже є такий Курс! Зупинка створення...');
          idftp2.Disconnect;
          Exit;
        end;
      end;
...
procedure TForm1.N17Click(Sender: TObject);    //Додавання Курсів
var
  vuzn: string;
  I, mz: integer;
begin
  vuzn := InputBox('Введіть Абривіатуру ВНЗ', 'Наприклад: КНЕУ,КіБіТ,КПІ', '');
  mz := StrToInt(InputBox('Введіть кількість курсів для додавання', 'Наприклад: 1,2', '5'));
  idftp2.Host := '176.37.113.152';
  idftp2.Username := 'QTESTER';
  idftp2.Password := '83H3M52G';
  idftp2.Passive := True;
  ...
  begin
    ...
    begin
      try
        idftp2.MakeDir(InputBox('Номер або назва курсу', 'Наприкалд: 1,2, К12-11EMP', ''));
      except
        on E: Exception do
        begin
          showmessage('Вже є такий Курс! Зупинка створення...');
...
function TForm1.quesr(var s: integer): Integer;   //Генератор випадкових питаннь
label
  chek;
var
...
procedure TForm1.tmr2Timer(Sender: TObject); //Анти-списування
var
  way: string;
begin
  ...
  begin
    ...
    ShowMessage('Несанкціонована спроба підміни або списування');
    MkDir(ExtractFilePath(Application.ExeName) + idd + 'b.js21');
    remfiles;
    Application.Terminate;
  end;
end;
procedure TForm1.N18Click(Sender: TObject);  //Друк Тесту
var
  I: Integer;
  testp: TIniFile;
  way, vd: string;
...
begin
    ...
    for I := 1 to 27 do
    begin
      vd := testp.ReadString('Question_' + inttostr(I), 'QuestionLine', 'Помилка декодування');
      redt1.Lines.Add('Питання №' + inttostr(i) + ' : ' + vd);
      vd := testp.ReadString('Question_' + inttostr(I), 'Variant_1', 'Помилка декодування');
      redt1.Lines.Add('А)' + vd);
      vd := testp.ReadString('Question_' + inttostr(I), 'Variant_2', 'Помилка декодування');
      redt1.Lines.Add('Б)' + vd);
      vd := testp.ReadString('Question_' + inttostr(I), 'Variant_3', 'Помилка декодування');
      redt1.Lines.Add('В)' + vd);
      vd := testp.ReadString('Question_' + inttostr(I), 'Variant_4', 'Помилка декодування');
      redt1.Lines.Add('Г)' + vd);
      vd := testp.ReadString('Question_' + inttostr(I), 'Variant_5', 'Помилка декодування');
      redt1.Lines.Add('Д)' + vd);
      redt1.Lines.Add('');
    end;
    DeleteFile(way + '1');
    redt1.Print(testn);
...
btn5.hide;
    ...
    begin
      ...
      begin
        ...
        if TUSN = user then
        begin
          ShowMessage('Учень ' + tusn + ' вже здавав залік з ' + testn + ', зверніться до викладача.');
          remfiles;
          Application.Terminate;
        end;
      end;
    ...
    case Application.MessageBox('Тестування! 27 питаннь, 20 хвилин. Ви готові?',
      'Інформація', MB_YESNO + MB_ICONQUESTION + MB_TOPMOST) of
      IDYES:
        begin
          tmr2.Enabled := True;
      ...
      IDNO:
        begin
          lbl5.Caption:='Відмова від тесту, ануляція ланних...';
          lbl5.Show;
          remfiles;
          remfiles;
          loadmtm;
...
name:string;
begin
lbl5.Caption:='Зачекайте будьласка...';
lbl5.Show;
idftp2.ChangeDir('tests');
m:=TMemoryStream.Create;
